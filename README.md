...
2. `chroot` in (native) and `pacman -S` the runtime: SDL2, mesa (v3d/vc4 GL), Python, git,
   NetworkManager, audio, fonts (see [`packages.txt`](packages.txt)).
3. `git clone` the app and **`pip install` it into a baked venv** at `/opt/drdro/app/.venv`. Native
...
4. Overlay the launcher + `drdro.service`, write our `config.txt`/`cmdline.txt`.
5. Assemble a 2-partition SD image (FAT boot + ext4 root) with `mke2fs -d` + `mtools` — no loop
   mounts.
...
- **CI:** `.github/workflows/build.yml` (run on demand via *workflow_dispatch*, or on push) builds on
  `ubuntu-24.04-arm` and uploads `drdro-arch-rpi-aarch64.img.zst`.
  ```
  gh workflow run build-arch -R bartei/drdro-arch
  gh run download <run-id> -R bartei/drdro-arch -n drdro-arch-rpi-aarch64
  zstd -d drdro-arch-rpi-aarch64.img.zst && sudo dd if=drdro-arch-rpi-aarch64.img of=/dev/sdX bs=4M conv=fsync status=progress
  ```
...
- ALARM `rpi-aarch64` covers Pi 3/4/5 from one tarball; this image targets that family. Verify GL is
  **V3D/Broadcom** (not `llvmpipe`) on first boot.
- Not hardware-tested yet — first CI run is shakeout (package names, ALARM boot-file layout).
